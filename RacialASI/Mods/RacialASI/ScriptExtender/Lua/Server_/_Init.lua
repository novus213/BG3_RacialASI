...
local function writing()
    ...
    if success then
        BasicPrint("============> RacialASI_Progression.json saved successfully.","INFO", nil, nil, true)
    else
        BasicError(string.format("============> Failed to save RacialASI_Progression.json: %s", errorMessage))
    end
end
...
local function reading()
    ...
    if not status then
        BasicError(string.format("============> Failed to load RacialASI_Progression.json: %s", file_contents)) -- file_contents will contain the error message
        return nil
    end

    if not file_contents then
        BasicError("============> RacialASI_Progression.json not found. Creating configuration at Mods Folder")
        ...
        if not status or not file_contents then
            BasicWarning("============> ERROR: Failed to load config file even after creating default.")
            return nil
        end
    end
    ...
    if not success then
        BasicError(string.format("============> ERROR: Failed to parse config file. Invalid JSON format: %s", parsed_data)) -- parsed_data will contain the error message
        return nil
    end
...
local function loadConfiguration()
    ...
    if not config then
        BasicError("============> ERROR: Failed to load configuration file")
    end
    return config
end
