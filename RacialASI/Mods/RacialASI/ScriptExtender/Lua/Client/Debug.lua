...
local function CheckTableUuid(t1, name, printo)
  for _, uuidIn in pairs(t1) do
    ...
    if printo == "warn" then
      BasicWarning(string.format("TYPE %s , modData.Info.Directory : %s , modData.Info.ModuleUUID : %s, modData.Info.Name : %s, modData.Info.Author : %s",
        name, modDataE.Info.Directory, modDataE.Info.ModuleUUID, modDataE.Info.Name, modDataE.Info.Author))
    else
      BasicPrint(string.format("TYPE %s , modData.Info.Directory : %s , modData.Info.ModuleUUID : %s, modData.Info.Name : %s, modData.Info.Author : %s",
        name, modDataE.Info.Directory, modDataE.Info.ModuleUUID, modDataE.Info.Name, modDataE.Info.Author))
    end
  end
end
...
local function compare(t1, t2)
  ...
  for _, uuidI in pairs(t1) do
    for _, uuidIn in pairs(t2) do
      if uuidI ~= uuidIn then
        table.insert(tempUuid, uuidI)
      end
    end
  end
  return tempUuid
